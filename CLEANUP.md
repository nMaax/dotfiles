...
**What breaks:** nothing. The Lua config has since survived weeks of daily use and is the standing
config now (`CLAUDE.md` §4) — there is no more reason to keep a hyprlang rollback path open.

---

## 2. Noctalia v4 (`noctalia-shell`)

v4 was kept installed alongside v5 for a while as a fallback while v5 was being configured. v5 has
since proven stable, so v4 has been fully removed — package, config and script references, below.
...
**`noctalia-qs`** is already gone too — it had no remaining dependents once §8 removed
`quickshell-overview-git` alongside `noctalia-shell` in the same transaction, so it cascaded away on
its own rather than needing a separate removal step.

**What broke:** the v4 bar layout, pinned apps, enabled plugins and colorschemes are gone for good.
v5 had no migration path from v4 — this was only done once v5 was fully configured to taste.
